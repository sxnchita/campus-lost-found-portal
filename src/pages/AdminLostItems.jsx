...
function AdminLostItems() {
  ...
  const filteredItems = lostItems.filter((item) => {
    const matchesSearch =
      ...
      item.lostLocation?.toLowerCase().includes(search.toLowerCase());
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="bg-white border border-purple-100 rounded-3xl shadow-xl p-8 mb-8">
          ...
          <p className="text-slate-600 mt-3">
            Review, approve, reject, and search all reported lost items.
          </p>
        </div>
  ...
  <div className="bg-purple-50 border border-purple-100 rounded-2xl p-4 mb-4">
    <p><b>👤 Reported By:</b> {item.userName || item.reportedBy || item.name || "N/A"}</p>
    <p>
      <b>📅 Reported On:</b>{" "}
      {item.createdAt
        ...
        : item.reportedDate || item.date || "N/A"}
    </p>
  </div>

  <p><b>Location:</b> {item.lostLocation}</p>
                  <p><b>Color:</b> {item.color || "N/A"}</p>
                  <p><b>Model:</b> {item.model || "N/A"}</p>
                  <p><b>Description:</b> {item.description || "N/A"}</p>
                  <p><b>Special Features:</b> {item.specialFeatures || "N/A"}</p>
                </div>

                {item.status === "PENDING_APPROVAL" && (
                  <div className="flex gap-3 mt-6">
                    <button
                      onClick={() => approveLostItem(item.lostItemId)}
                    ...
                    <button
                      onClick={() => rejectLostItem(item.lostItemId)}
